...
- **Okno B (Ekran 2)** — pełnoekranowa, przewijana siatka równych kafelków 3:2 ze wszystkimi zdjęciami z sesji, najnowsze zawsze na górze. Miniatury są równo przycinane i dekodowane pod faktyczny rozmiar kafelka (także przy skalowaniu DPI), a kliknięcie pokazuje wybrane zdjęcie w Oknie A; siatka pozostaje cały czas widoczna. Zdjęcie widoczne w podglądzie ma w siatce ramkę w kolorze akcentu i jest do niej dowożone przy przechodzeniu strzałkami. Gdy automatyczny podgląd jest włączony, aplikacja wraca do najnowszego zdjęcia dopiero po ustawionym czasie.
...
- **Rozbudowane ustawienia i diagnostyka** — `Ctrl+P` otwiera okno podzielone na zakładki: **Ekrany** (tryb widoków, przypisanie monitorów), **Folder i galeria** (obserwowany folder, kolumny siatki, cache miniaturek, filtr zdjęć), **Podgląd** (automatyczne pokazywanie najnowszego zdjęcia i czas zatrzymania wybranego, EXIF, histogram, ostrzeżenia ekspozycji), **Udostępnianie** (hotspot, Wi‑Fi, kody QR, rozmiar i jakość pobieranych zdjęć), **Wygląd** (motyw, skala UI, język, aktualizacje), **Diagnostyka** oraz **O aplikacji**. Diagnostyka pokazuje stan hotspotu, adres IP, próby naprawy, błąd sterownika/AP+STA, aktywne pobrania i dostępność folderu. Zmiany wizualne są stosowane na żywo; restart następuje tylko dla folderu, ekranów lub hotspotu.
- **Pobieranie zdjęć na telefon gościa (opcjonalne)** — komputer tworzy odizolowany hotspot Wi‑Fi; dwa kody QR służą do dołączenia do sieci i pobrania dokładnie zdjęcia widocznego w podglądzie. Limit równoległych pobrań chroni aplikację, start ma maksymalnie trzy kontrolowane próby, a po pięciu minutach bez pobierania hasło sesji jest zmieniane.
- **Rozmiar i format pobieranych zdjęć** — w zakładce Udostępnianie wybierasz dłuższy bok (oryginalna rozdzielczość, 3840, 2560, 2048, 1600 lub 1024 px) oraz jakość JPEG (40–100%). Domyślnie gość dostaje oryginalny plik bajt w bajt, bez rekompresji. Po włączeniu przetwarzania pliki PNG pozostają PNG, a wszystkie pozostałe — w tym RAW, TIFF i HEIC — trafiają do telefonu jako JPEG z poprawioną nazwą i typem MIME. Zdjęcia mniejsze od wybranego limitu nie są rozciągane, pliki źródłowe nigdy nie są modyfikowane, a przekodowana kopia powstaje wyłącznie w pamięci i jest współdzielona między gośćmi skanującymi ten sam kod QR.
...
- **Skróty** — `Ctrl+P` ustawienia, `Esc` reset powiększenia, `Home` najnowsze zdjęcie, `←`/`→` poprzednie/następne, `Ctrl+M` minimalizacja, `F11` pełny ekran/tryb okienkowy. Skróty działają w obu oknach, niezależnie od tego, które ma fokus — również w układzie jednoekranowym. Strzałki przechodzą po zdjęciach widocznych w siatce (z uwzględnieniem filtra), a przy pustym podglądzie pierwsze naciśnięcie pokazuje najnowsze zdjęcie.
