...
- **Ustawienia** — dostępne przy starcie i w dowolnym momencie skrótem `Ctrl+P`. Pozwalają m.in. pokazać lub ukryć panel parametrów zdjęcia, włączyć/wyłączyć automatyczny hotspot, ustawić wielkość QR i czas ręcznego podglądu od 3 do 900 sekund. Po zapisaniu aplikacja uruchamia się ponownie.
- **Pobieranie zdjęć na telefon gościa (opcjonalne)** — komputer tworzy odizolowany hotspot WiFi; w sidebarze Okna B widoczne są dwa kody QR (dołączenie do sieci + pobranie zdjęcia aktualnie wyświetlanego w Oknie A). Start automatycznie ponawia się przy przejściowym błędzie Windows, a poprzedni stan hotspotu jest porządkowany przed konfiguracją. Sieć wyłącza się automatycznie po 5 minutach bezczynności i wznawia przy nowym zdjęciu.
